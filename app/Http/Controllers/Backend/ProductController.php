...
class ProductController extends Controller
{
    ...
    {
        $products=Product::with('user','category','brand')->latest()->paginate(10);
        return view('backends.pages.product.index',compact('products'));
    }
    ...
    public function create()
    {
        ...
        $brands     = Brand::where(['status'=>'public'])->latest()->get();
        return view('backends.pages.product.create', compact('category','brands'));
    }
    ...
    {
        $request->validate([
            'title' => ['required', 'string', 'max:255', 'unique:'.Product::class],
            'thumb'=> ['required', 'mimes:jpeg,jpg,png,gif', 'max:10000'] // max 10000kb
        ]);
        $id = IdGenerator::generate(['table'=>'products','field'=>'product_id','length' => 10, 'prefix' =>'PID-']);
        // ======= store data ======= //
        $data=$request->all();
        // ======= store data ======= //
        if($request->hasFile('thumb')) {
            ...
            $img->toJpeg(80)->save(base_path('public/uploads/products/p_thumb_'.$name_gen));
            $save_url = 'uploads/products/p_thumb_'.$name_gen;
            $data['thumb']=$save_url;
        } // endif ==========
        // ======= store data ======= //
    ...
    public function edit(string $id)
    {
        ...
        $category   = Category::where(['status'=>'public'])->latest()->get();
        $brands     = Brand::where(['status'=>'public'])->latest()->get();
        return view('backends.pages.product.edit', compact('getData','category','brands'));
    }
    ...
    public function status(string $id)
    {
        ...
        $msgstatus=Product::where('id', $id)->update(['status'=>$status]);
        if($msgstatus){
